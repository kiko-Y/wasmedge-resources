...
相对于 C 语言，wasm 在保持运行效率的基础上还可以有更好的可移植性以及安全性。

wasm 最重要的两个特性：**可移植性**、**安全性**

先说可移植性，c 代码编译成 wasm 之后，可以通过 wasm runtime 在任何系统上运行。而在此之前，c 代码，每次到一个新的系统上，就需要重新编译源码。

而安全性则表现在 wasm 的 `sandbox` 环境，用户需要给 wasm 指定可以使用的**系统资源**和可以使用的**方法**，除此之外的系统资源wasm 是访问不到的。虽然用户依然可以把所有系统资源的权限交给 wasm，但是 wasm 这样做还是给了用户选择性，一定程度上提高了安全性。
